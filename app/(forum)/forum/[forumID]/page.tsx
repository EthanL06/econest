...
import { getForumById, getUserById } from "@/config/routes";
...
const Page = ({ params }: Props) => {
  ...
  useEffect(() => {
    const fetchForum = async () => {
      const forumData = await getForumById(params.forumID);
    ...
    fetchForum();
  }, [params.forumID, router]);

  return (
    <div>
      {!forum && <Loader />}
      {forum && <ForumPage user={userData} forum={forum} />}
    </div>
  );
};
